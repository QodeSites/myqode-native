...
export default class MyQode extends React.Component {
  ...
  obVals() {
    ...
    const tidyNom = (f, t) => (f === 'mob' ? String(t).replace(/\D/g, '').slice(0, 10) : f === 'alloc' ? String(Math.min(100, parseInt(String(t).replace(/\D/g, '') || '0', 10) || 0) || (String(t).replace(/\D/g, '') ? '0' : ''))
      : f === 'name' || f === 'guardian' ? String(t).replace(/[^A-Za-z\u00C0-\u024F .'-]/g, '').slice(0, 80) : String(t).slice(0, 60));
    const upd = (i, f) => t => { const noms = OB.noms.map((n, j) => j === i ? { ...n, [f]: tidyNom(f, t) } : n); setOb({ noms, nomErr: '' }); };
